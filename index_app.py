...
def is_ollama_api(base_url: str) -> bool:
    ...
    try:
        response = requests.get(f"{normalize_api_base(base_url)}/api/tags", timeout=5)
...
def get_openai_compatible_models(base_url: str) -> List[str]:
    ...
    try:
        ...
        response = requests.get(f"{normalized_base}/v1/models", timeout=5)
        response.raise_for_status()
...
def get_local_models(base_url: str) -> List[str]:
    ...
    normalized_base = normalize_api_base(base_url)
    if is_ollama_api(normalized_base):
        return get_ollama_models(normalized_base)
    else:
        return get_openai_compatible_models(normalized_base)
