...
import subprocess
import argparse
from graphrag.llm.openai.create_openai_client import create_openai_client, normalize_api_base
...
def normalize_api_base(api_base: str) -> str:
    """Normalize the API base URL by removing trailing slashes and /v1 suffix."""
...
async def setup_llm_and_embedder(settings):
    ...
    try:
        llm = await create_openai_client(settings)

        token_encoder = tiktoken.get_encoding("cl100k_base")

        embeddings_settings = settings.copy()
        embeddings_settings['api_base'] = settings['embeddings_api_base']
        text_embedder = await create_openai_client(embeddings_settings)
...
async def run_direct_chat(request: ChatCompletionRequest) -> ChatCompletionResponse:
    try:
        ...
        full_url = f"{normalize_api_base(LLM_API_BASE)}/chat/completions"
